<div class="members-section" data-project-id="<?= (int)$projectId ?>">
  <div style="display:flex;flex-direction:column;gap:6px;">
    <?php foreach ($members as $m): ?>
      <div class="member-chip" data-user-id="<?= (int)$m['id'] ?>"
        ...
        <div class="ini" style="width:24px;height:24px;font-size:10px;background:var(--ink);">
          <?= e(mb_substr($m['name'], 0, 2)) ?>
        </div>
  ...
  <?php if ($canEdit): ?>
    <div class="member-add" style="margin-top:12px;position:relative;">
      <input class="input" type="text" placeholder="+ Add member by name or email" data-member-search>
      <div class="dropdown" data-member-dropdown
           style="display:none;position:absolute;top:100%;left:0;right:0;background:var(--paper);border:1px solid var(--rule);max-height:200px;overflow-y:auto;z-index:10;margin-top:4px;border-radius:4px;">
      </div>
